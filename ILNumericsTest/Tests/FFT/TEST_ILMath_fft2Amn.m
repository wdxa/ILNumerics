...
sCName = sprintf('TEST_ILMath_fft2_Amn'); 
...
for d = (1:length(dims))
    ...
    for m = 0:(size(A,1)+1) 
        for n = 0:(size(A,2)+1)
            ...
            fprintf(fid,'TEST_ILMath_fft_HELPER.forwBackwCheck2Dmn(A, Result,ResRA,ResIA,%d,%d);\r\n',m,n); 
        end
    end
    fprintf(fid,'}\r\n');  
end 
