...
sForwBackw1D = ['private static void forwBackwGenCheck(IILFFT fft, ILArray<complex> A, ILArray<complex> Result, int dim, double mult) { ' 13 10 ...
    ...
    '    throw new Exception("invalid value");}' 13 10 ];
sForwBackwND = ['private static void forwBackwGenCheckND(IILFFT fft, ILArray<complex> A, ILArray<complex> Result, int d, double mult) { ' 13 10 ...
'    try {' 13 10 ...
...
function printADef(fid,A,prec)
    ar = sprintf('%1.15f,',real(A(:))); 
    ar = ar(1:end-1);
    ai = sprintf('%1.15f,',imag(A(:))); 
    ai = ai(1:end-1);
    fprintf(fid,'ResultR = new %s[]{%s};\r\n',prec,ar);
    fprintf(fid,'ResultI = new %s[]{%s};\r\n',prec,ai);
