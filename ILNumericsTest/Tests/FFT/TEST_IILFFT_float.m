...
sForwBackw1D = ['private static void forwBackwGenCheck(IILFFT fft, ILArray<float> A, ILArray<fcomplex> Result, int dim, float mult) { ' 13 10 ...
    'ILArray<fcomplex> B = fft.FFTForward1D(A, dim);'  13 10 ...
    'if (ILMath.sumall(ILMath.abs(Result - B))/A.Dimensions.NumberOfElements / A.Dimensions[dim] > (double)ILMath.MachineParameterFloat.eps * mult)'  13 10 ...
    '    throw new Exception("invalid value");' 13 10 ...
    'ILArray<float> ResultR = fft.FFTBackwSym1D(B,dim);' 13 10 ... 
    'if (ILMath.sumall(ILMath.abs(ResultR - A))/A.Dimensions.NumberOfElements / A.Dimensions[dim] > (double)ILMath.MachineParameterFloat.eps * mult)' 13 10 ...
    '   throw new Exception("invalid value");' 13 10 ...
    'B = fft.FFTBackward1D(B, dim);'  13 10 ...
    'if (ILMath.sumall(ILMath.abs(ILMath.tofcomplex(A) - B))/A.Dimensions.NumberOfElements / A.Dimensions[dim] > (double)ILMath.MachineParameterFloat.eps * mult)'  13 10 ...
    '    throw new Exception("invalid value");}' 13 10];
sForwBackwND = ['private static void forwBackwGenCheckND(IILFFT fft, ILArray<float> A, ILArray<fcomplex> Result, int d, float mult) { ' 13 10 ...
'    try {' 13 10 ...
'    ILArray<fcomplex> B = fft.FFTForward(A, d);' 13 10 ...
'    float errMult = A.Dimensions.NumberOfElements; ' 13 10 ...
'    for (int i = 0; i < d; i++) errMult *= A.Dimensions[i]; ' 13 10 ...
'    if (ILMath.sumall(ILMath.abs(Result - B))/errMult > (double)ILMath.MachineParameterFloat.eps * mult)' 13 10 ...
'        throw new Exception("invalid value");' 13 10 ...
'    ILArray<float> ResultR = fft.FFTBackwSym(B,d);' 13 10 ...
'    if (ILMath.sumall(ILMath.abs(ResultR - A))/errMult > (double)ILMath.MachineParameterFloat.eps * mult)' 13 10 ...
'       throw new Exception("invalid value");' 13 10 ...
'    B = fft.FFTBackward(B, d);' 13 10 ...
'    if (ILMath.sumall(ILMath.abs(ILMath.tofcomplex(A) - B))/errMult > (double)ILMath.MachineParameterFloat.eps * mult)' 13 10 ...
'        throw new Exception("invalid value");' 13 10 ...
...
function printResDef(fid,Result,prec) 
    resR = sprintf('%1.7ff,',real(Result(:))); 
    resR = resR(1:end-1); 
    fprintf(fid,'ResultR = new %s[]{%s};\r\n',prec,resR);
    resI = sprintf('%1.7ff,',imag(Result(:))); 
    resI = resI(1:end-1); 
