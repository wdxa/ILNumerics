...
sForwBackw1D = ['private static void forwBackwGenCheck(IILFFT fft, ILArray<fcomplex> A, ILArray<fcomplex> Result, int dim, float mult) { ' 13 10 ...
    ...
    'if (ILMath.sumall(ILMath.abs(A - B))/A.Dimensions.NumberOfElements / A.Dimensions[dim] > (double)ILMath.MachineParameterFloat.eps * mult)'  13 10 ...
    '    throw new Exception("invalid value");}' 13 10 ];
sForwBackwND = ['private static void forwBackwGenCheckND(IILFFT fft, ILArray<fcomplex> A, ILArray<fcomplex> Result, int d, float mult) { ' 13 10 ...
'    try {' 13 10 ...
...
for d = (1:length(dims))
    A = (1:(prod(dims{d}))) + j*(prod(dims{d}):-1:1); 
    A = reshape(A,dims{d}); 
    ...
    fprintf(fid,'ILArray<%s> ResultR,ResultI; \r\n',sPrecType);
    fprintf(fid,'ILArray<%s> A; \r\n',sInType);
    fprintf(fid,'ILArray<%s> B, Result;\r\n',sOutType); 
    printADef(fid,A,sPrecType); 
    % iterate over dimensions
    for i = 1:length(dims{d})
        % test 1d
        Result = fft(A,[],i);
        printResDef(fid,Result,sPrecType);  
...
function printADef(fid,A,prec)
    ar = sprintf('%1.7ff,',real(A(:))); 
    ar = ar(1:end-1);
    ai = sprintf('%1.7ff,',imag(A(:))); 
    ai = ai(1:end-1);
    fprintf(fid,'ResultR = new %s[]{%s};\r\n',prec,ar);
    fprintf(fid,'ResultI = new %s[]{%s};\r\n',prec,ai);
    fprintf(fid,'A = ILMath.ccomplex(ResultR,ResultI);\r\n');
