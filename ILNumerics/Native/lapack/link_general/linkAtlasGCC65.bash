...
LapackName=lapack_gen

# set names for specific output files (see notes on MSVC lib.exe below)
defname=${LapackName}.def
dllname=${LapackName}.dll
gcclibname=${LapackName}-gcc.lib
msvclibname=${LapackName}-msvc.lib

# create the .def file used for linking Atlas .dlls
# uncomment or comment the various routines below as needed to get what you
# want in the DLL

#minimal set of Fortran exports needed to produce an Atlas callable by Matlab 6.5.1
#MatlabExports=(\
#dasum daxpy dcopy ddot dgbmv dgemm dgemv dger dnrm2 drot drotg dsbmv        \
#dscal dspmv dspr dspr2 dswap dsymm dsymv dsyr dsyr2 dsyr2k dsyrk dtbmv      \
#dtbsv dtpmv dtpsv dtrmm dtrmv dtrsm dtrsv dzasum dznrm2 idamax izamax       \
#xerbla zaxpy zcopy zdotc zdotu zdscal zgbmv zgemm zgemv zgerc zgeru zhbmv   \
#zhemm zhemv zher zher2 zher2k zherk zhpmv zhpr zhpr2 zrotg zscal zswap      \
#zsymm zsyr2k zsyrk ztbmv ztbsv ztpmv ztpsv ztrmm ztrmv ztrsm ztrsv)

#minimal set of Fortran exports needed to produce an Atlas callable by Matlab 6.0 and 6.1
#MatlabExports=(\
...
FortranExports=(\
#single precision routines not found: scsum sdsdot srotm srotmg 
	saxpy scnrm2 scopy sdot sgbmv sgemm \
	sgemv sger snrm2 srot srotg ssbmv sscal \
	sspmv sspr sspr2 sswap ssymm ssymv ssyr ssyr2 ssyr2k \
	ssyrk stbmv stbsv stpmv stpsv strmm strmv strsm strsv \
#double precision routines  not found: drotmg dsdot 
	dasum daxpy dcabs1 dcopy ddot dgbmv dgemm dgemv \
	dger dnrm2 drot drotg dsbmv dscal \
...
#other stuff
	xerbla \
# LAPACK interface subset  ----- add needed lapack/blas functions here!!! <=========================== 
	dgemm dgeqrf dgeqp3 dgesdd dgesvd dgetrf dgetri dormqr dorgqr dpotrf dpotri dgetrs dtrtrs dsyevr dgelsy dgeevx dpotrs dsygv \
	sgemm sgeqrf sgeqp3 sgesdd sgesvd sgetrf sgetri sormqr sorgqr spotrf spotri sgetrs strtrs ssyevr sgelsy sgeevx spotrs ssygv \
	cgemm cgeqrf cgeqp3 cgesdd cgesvd cgetrf cgetri cungqr	      cpotrf cpotri cgetrs ctrtrs cheevr cgelsy cgeevx cpotrs chegv \
	dgemm zgeqrf zgeqp3 zgesdd zgesvd zgetrf zgetri zungqr	      zpotrf zpotri zgetrs ztrtrs zheevr zgelsy zgeevx zpotrs zhegv \
	xerbla  lsame ilaenv  
)
#create exports file
...
# libraries
CLIBPATH="/lib"
mingwclib="$CLIBPATH/gcc/i686-pc-mingw32/3.4.4/libg2c.a $CLIBPATH/mingw/libmoldname.a $CLIBPATH/mingw/libmsvcrt.a"
...
LIBPATH=`cygpath -d "C:\Programme\Microsoft Visual Studio\VC98\bin"`
VBPATH=`cygpath -d "C:\Programme\Microsoft Visual Studio\Common\MSDev98\Bin"`
