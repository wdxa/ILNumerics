...
// DO NOT EDIT INSIDE THIS REGION !! CHANGES WILL BE LOST !! 
        ...
        public ILArray< fcomplex >  FFTBackward1D (ILArray< fcomplex > A, int dim) {
            ...
            string hash = hashPlan( MKLValues.SINGLE ,MKLValues.COMPLEX, 1, A.Dimensions[dim]); 
            IntPtr descriptor = IntPtr.Zero;
            int error, inDim = A.Dimensions[dim];   
            // try to reuse existing descriptor
            lock (_lockobject) {
                if (!m_descriptors.TryGetValue(hash, out descriptor)) {
                    error = MKLImports.DftiCreateDescriptor(ref descriptor, MKLValues.SINGLE ,MKLValues.COMPLEX, 1, inDim);
                    if (isMKLError(error)) {
                        throw new ILInvalidOperationException ("error creating descriptor: " + MKLImports.DftiErrorMessage(error)); 
                    }
                    m_descriptors[hash] = descriptor; 
...
<hycalper>
    ...
    <type>
        ...
        <destination></destination>
        <destination><![CDATA[if (nDims > 3) return FFTForward(ILMath.tofcomplex(A),nDims);]]></destination>
        <destination></destination>
        <destination></destination>
        <destination></destination>
    </type>               
...
// DO NOT EDIT INSIDE THIS REGION !! CHANGES WILL BE LOST !! 
        ...
        public ILArray< fcomplex >  FFTBackward (ILArray< fcomplex > A, int nDims) {
            ...
            int error = -1;   
            int[] tmpDims = A.Dimensions.ToIntArray(nDims+1);
            string hash = hashPlan( MKLValues.SINGLE , MKLValues.COMPLEX, nDims, tmpDims);
            lock (_lockobject) {
            // try to reuse existing descriptor
                if (!m_descriptors.TryGetValue(hash, out descriptor)) {
                    error = MKLImports.DftiCreateDescriptor(ref descriptor,  MKLValues.SINGLE , MKLValues.COMPLEX, nDims, tmpDims);
                    if (isMKLError(error)) {
                        throw new ILInvalidOperationException ("error creating descriptor: " + MKLImports.DftiErrorMessage(error)); 
                    }
                    m_descriptors[hash] = descriptor; 
...
// DO NOT EDIT INSIDE THIS REGION !! CHANGES WILL BE LOST !! 
        ...
        public ILArray< float > FFTBackwSym(ILArray< fcomplex > A, int nDims) {
            ...
            int error = -1;   
            int[] tmpDims = A.Dimensions.ToIntArray(nDims+1);
            string hash = hashPlan( MKLValues.SINGLE , MKLValues.REAL, nDims, tmpDims);
            // try to reuse existing descriptor
            lock (_lockobject) {
                if (!m_descriptors.TryGetValue(hash, out descriptor)) {
                    error = MKLImports.DftiCreateDescriptor(ref descriptor,  MKLValues.SINGLE , MKLValues.REAL, nDims, tmpDims);
                    if (isMKLError(error)) {
                        throw new ILInvalidOperationException ("error creating descriptor: " + MKLImports.DftiErrorMessage(error)); 
                    }
                    m_descriptors[hash] = descriptor; 
