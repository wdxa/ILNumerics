...
// DO NOT EDIT INSIDE THIS REGION !! CHANGES WILL BE LOST !! 
    ...
    /// <para>This class is deprecated and will be removed in a future release. Use the alias ILMatFile instead! MatFile is here for backward compatibility only!</para>
    /// </remarks>
        ...
        private void write(BinaryWriter fileout) {
            try
            {
                ...
                foreach (KeyValuePair<string,ILBaseArray> ba in m_data) {
                    ...
                    int[] dimensionSubelement = createDimensionSubelement(arr); 
                    byte[] nameElememData; int nameElemType;
                    createNameSubelement(arr,out nameElemType, out nameElememData); 
                    // write subarray header               
                    // mxarray subelement type
                    fileout.Write((int)MatFileType.miMATRIX);  
                    // determine length of single array element
...
// DO NOT EDIT INSIDE THIS REGION !! CHANGES WILL BE LOST !! 
        ...
        /// create name subelement for Matfile storage - padded to 8 byte border
        /// </summary>
        /// <param name="arr">arr with name property</param>
        /// <param name="type">miINT8</param>
        /// <param name="data">return name subelemen </param>
        private void createNameSubelement(ILBaseArray arr, out int type, out byte[] data) {
            int len = arr.Name.Length; 
            if  (len % 8 != 0) 
                len += ( 8 - len % 8 );
            data = new byte[len];        
            for (int i = 0; i < arr.Name.Length; i++) {
                data[i] = (byte)arr.Name[i];         
            }
            type = (int)MatFileType.miINT8; 
            return;  
        }
